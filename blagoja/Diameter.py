...
destination_path_nodes="/home/ubuntu/NSProject/dataset/dataset-new/stage/Diameter"
destination_path_stats="/home/ubuntu/NSProject/dataset/dataset-new/stage/Stats/Diameter"
...
for x in listSrcDir:
    if x.find("date=2022")  != -1:
        ...
        if not (dirname in listDesDir):
            ...
            #df_diameter_nodes.show()
            #df_diameter_stats.show()

            #Write the rows in a directory
            #df_diameter_nodes.write.option("header", True).mode('overwrite').csv(destination_path_nodes+"/"+dirname)
            df_diameter_stats.write.option("header", True).mode('overwrite').csv(destination_path_stats+"/"+dirname)
