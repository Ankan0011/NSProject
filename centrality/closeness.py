...
destination_path_nodes="/home/ubuntu/NSProject/dataset/dataset-new/stage/Closeness"
destination_path_stats="/home/ubuntu/NSProject/dataset/dataset-new/stage/Stats/Closeness"

# Variables
e1_cols=["SenderId","TargetId","Year_no","Week_no"]
e1_final=["from","to","relationship"]

final_columns_nodes = ["closeness_centrality", "time_week"]
final_columns_stats = ["mean_closeness", "std_closeness", "gini_coefficient", "time_week" ]
...
for x in listSrcDir:
    if x.find("date=2022")  != -1:
        ...
        if not (dirname in listDesDir):
            ...
            close_nodes = nx.closeness_centrality(G)
            close_mean = np.array(list(close_nodes.values())).mean()
            close_std = np.array(list(close_nodes.values())).std()
            close_gini = gini(np.array(list(close_nodes.values())))


            #close_nodes = [(str(close_nodes), dirname.split("=")[-1])]
            close_stats = [(str(close_mean), str(close_std), str(close_gini), dirname.split("=")[-1])]
            #df_close_nodes = spark.createDataFrame(close_nodes).toDF(*final_columns_nodes)
            df_close_stats = spark.createDataFrame(close_stats).toDF(*final_columns_stats)
            ...
            #df_close_nodes.write.option("header", True).mode('overwrite').csv(destination_path_nodes+"/"+dirname)
            df_close_stats.write.option("header", True).mode('overwrite').csv(destination_path_stats+"/"+dirname)
